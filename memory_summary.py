...
_TOOL_SUMMARY_MAX_CHARS = int(os.getenv("LM_PROXY_MEMORY_TOOL_SUMMARY_MAX_CHARS", "400"))
...
def _extract_bullets(text: str) -> List[str]:
    ...
    decision_pat = re.compile(r"(?i)(decided|choosing|choose|will use|approach|strategy|switched to|use this)")
    action_pat = re.compile(r"(?i)(next step|todo|action|plan|need to|should|must|try this)")
...
def compact_tool_output(tool_name: str, raw_output: str) -> Dict[str, str]:
    ...
    _debug("tool_output_compacted", tool=tool_name, raw_len=len(raw_output), compact_len=len(compact))
...
def parse_summary_text(summary_text: str) -> Dict[str, List[str]]:
    ...
    decision_pat = re.compile(r"(?i)(decided|choose|choosing|will use|approach|strategy|switched)")
...
def extract_summary_facts(summary_text: str, max_items_per_section: int = 5) -> List[Dict[str, str]]:
    """
...
async def _call_summarizer_llm(prompt: str) -> Optional[str]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            r = await client.post(f"{_SUMMARIZER_BASE_URL}/v1/chat/completions", json=payload)
    ...
    if _SUMMARIZER_MODEL:
        ...
        if result:
            ...
            if parsed is not None:
                merged = _merge_summary_dicts(previous_structured, normalize_summary_dict(parsed))
...
def compact_turn_content(role: str, content: str, max_chars: int = 600) -> str:
    ...
    if role == "tool":
        return compact_text_deterministic(content, min(max_chars, _TOOL_SUMMARY_MAX_CHARS))
