...
| `get_app_flow_summary` | **Niche/High** | Keep experimental until a real indexed full-stack repo validates positive end-to-end paths. | **Fixed/Tested**: Route catalog and literal-call fallbacks prefer route-qualified paths. Empty results now report source/edge coverage, identify missing graph evidence, and route users to `get_flow_summary(mode='auto')`; the diagnostic is live-tested. |
