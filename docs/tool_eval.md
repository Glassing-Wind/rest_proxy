...
| `index_workspace` | **Critical** | None. | **Fixed/Tested**: MCP remains asynchronous, while the documented CLI waits for persisted terminal state, reports final status, and exits nonzero on failure or timeout. |
