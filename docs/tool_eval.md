...
| `get_code_communities` | **High** | Continue comparing live output against real onboarding workflows before promotion. | **Fixed/Tested**: Opens with a Community Summary, dominant concerns, first cluster recommendation, and omitted-tail accounting. Cluster filtering excludes test/example/benchmark/docs surfaces and labels prefer indexed file roles, retaining path heuristics only for legacy records. |
