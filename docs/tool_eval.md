...
| `index_workspace` | **Critical** | None. | **Fixed/Tested**: MCP remains asynchronous, while the documented CLI waits for persisted terminal state, reports final status, and exits nonzero on failure or timeout. Persisted lock owners retain/backfill job IDs so competing processes can report a monitorable job instead of `unknown`. |
