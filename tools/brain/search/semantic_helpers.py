...
def _result_label(result: dict, index: int) -> str:
...
def compact_duplicate_analysis(contract: dict, results: list[dict]) -> dict:
    ...
    for pair in contract.get("pairs") or []:
        ...
        item = {
            ...
            "right": right,
            "left_path": (
                _result_label(results[left], left)
                if isinstance(left, int) and 0 <= left < len(results)
                else None
            ),
            "right_path": (
                _result_label(results[right], right)
...
def compact_rerank_contract(contract: dict, results: list[dict]) -> dict:
    ...
    return {
        ...
        "ordered_results": [
            {"index": idx, "path": _result_label(results[idx], idx)}
            for idx in kept
            if isinstance(idx, int) and 0 <= idx < len(results)
        ],
        "suppressed_results": [
